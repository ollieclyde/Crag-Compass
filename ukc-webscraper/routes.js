...
(async () => {
  try {
    ...
    for (const crag of crags.data) {
      const cragRoutesExist = await axios.get(`http://localhost:3000/crags/cragRoutesExist/cragID/${crag.cragID}`);
      ...
      if (cragRoutesExist.status === 200) {
        ...
        for (const route of routes) {
          ...
          if (route.grade !== 'Unknown' && route.name !== 'Unknown') {
            console.log('test')
            console.log(route.name, route.grade, route.climbingTypeID, route.stars, route.logs)
